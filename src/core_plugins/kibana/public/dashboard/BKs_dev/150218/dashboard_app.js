...
app.service('PersistTitleAndId', function(){
   var title = "None";
   var Id = "None";
   var Counter = 0;
...
app.directive('dashboardApp', function ($injector) {
  ...
  return {
    ...
    controller: function ($scope, $rootScope, $route, $routeParams, $location, getAppState, $compile, dashboardConfig, PersistTitleAndId) {
      ...
      $scope.dashboardList = $route.current.locals.allDash;
      $scope.selected = $route.current.params.id;

      //Kp Scope variables and functions
       
       $scope.createDinamicId = function (){
           len1 = 5;
           len2 = 4;
           function getRandomInt(min, max) {
              ...
              JointList = [];
              for (i = 0;i<len2; i++){
                  var RanArray = Array.apply(null, Array(len1)).map(Number.prototype.valueOf,0);
                  RanArray = RanArray.map(function(element){
                  return  Math.round((Math.pow(36, 2) - Math.random() * Math.pow(36, 1))).toString(36).slice(1)});
           
           JointList.push(RanArray.join("") + "-")};
           JoinList = JointList.join("");
           alert(JoinList.slice(0, ((len2*(len1+1) - 1))));
           $scope.RandomId = JoinList.slice(0, ((len2*(len1+1) - 1)));
         };



       $scope.setTabCss = function (){
              var SelId = PersistTitleAndId.getId();
              var SelTitle = PersistTitleAndId.getTitle();    
              var Elements = document.querySelectorAll(".CustomTab");
              document.getElementById("test").innerText = SelId;
              Elements.forEach(function(element) {
                 var QueryEl = $(element);  
                ...
                function(){     
                 ...
                 if (element.innerText == SelTitle || element.id == SelId  ){
		  QueryEl.css({
			  "background-color":"#bebebe",
                          "border-bottom":"2px solid red"
			  });
      ...
      $scope.NavigateChangeColor = function (button, btn){
           ...
           function ScopeWitchIsSelected(ClickedElement, reptIndex){
               ...
               PersistTitleAndId.setTitle(title);
               PersistTitleAndId.setId(idIndex); 
           }
            index = selectId(button);
            document.getElementById('test').innerText = index; 
            ScopeWitchIsSelected(button, index);
            navigateTo(btn)     
       }
...
/*KP backup functions
   ...
   $scope.navigateTo = function (btn) {
         kbnUrl.change('dashboard/' + btn._id);
         return(true)
          }

      
      $scope.ChangeTabColor = function (button, btn){
        var title = button.innerText;
        var  Elements = document.querySelectorAll(".CustomTab");
        Elements.forEach(function(element) {
           var QueryEl = $(element);
