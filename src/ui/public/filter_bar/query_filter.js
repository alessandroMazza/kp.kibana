...
export function FilterBarQueryFilterProvider(Private, $rootScope, getAppState, globalState, config, fixedFilters) {
  
  //KP fixed filters except function and utility functions.
  function checkFilteredFiltersTag(Filters, blocked){
    if(!Filters){
       return(null);
    }
    var blockedFilters = [];
    var activeFilters = [];
      for( var i = 0; i < Filters.length; i++){
         if( Filters[i].meta.fixedFlag === true ){
           blockedFilters.push(Filters[i]);
         }
         else if (Filters[i].meta.fixedFlag === false){
  ...
  function applyToFixedFilterArray( arrayList, functionToApply, additionalArguments){
    for( var elCount = 0; elCount < arrayList.length; elCount++ ){
        additionalArguments.unshift(arrayList[elCount]);
        functionToApply.apply(this, additionalArguments)
    }
  }
  ...
  queryFilter.removeAll = function() {
    const appState = getAppState();
    var fixedFiltersAppState = checkFilteredFiltersTag(appState.filters, true);
    var fixedFiltersGlobalState = checkFilteredFiltersTag(globalState.filters , true);
    appState.filters =  fixedFiltersAppState;
    globalState.filters = fixedFiltersGlobalState;
   
   
  };


  queryFilter.unpinBlockedFilters = function(){
    const appState = getAppState();
    var fixedFilters = checkFilteredFiltersTag(appState.filters, true);
    //var freeFilters = checkFilteredFiltersTag(appState.filters, false);
    setTimeout(applyToFixedFilterArray( fixedFilters, queryFilter.pinFilter, [false]),5);
  }
    //END KP functions 
  ...
  */
  ...
  queryFilter.toggleFilter = function (filter, force) {
    // Toggle the disabled flag
    if(filter.meta.fixedFlag != true && filter.query){
        filter.meta.disabled = _.isUndefined(force) ? !filter.meta.disabled : !!force;
    }
    return filter;
  };

//END KP TOGGLE FILTER
...
//Start KP invertFilter function
  queryFilter.invertFilter = function (filter) {
    // Toggle the negate meta state
...
// END KP invertFilter function
   ...
   * @param {object} filter The filter to pin<div class="functionCaller" style= "display:none">{{setAllFixedFiltersFlagToFalse()}}</div>-
